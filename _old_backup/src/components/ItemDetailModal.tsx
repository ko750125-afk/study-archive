...
export function ItemDetailModal({ item, onClose }: Props) {
  const [newSubContent, setNewSubContent] = useState<string>("");
  const [showSubInput, setShowSubInput] = useState(false);
  const updateItem = useStore((s) => s.updateItem);
  const deleteItem = useStore((s) => s.deleteItem);

  const handleAddSubItem = async () => {
    if (!newSubContent.trim()) return;
    ...
    const newSub = {
      ...
      type: isUrl(newSubContent) ? "url" as const : "text" as const,
    };
        ...
        <div className="p-8 md:p-10">
          ...
          <div className="space-y-6 mb-10">
            ...
            <div className="space-y-5 pt-6 border-t border-outline-variant/20">
              <div className="flex items-center justify-between">
                ...
                {!showSubInput && (
                  <button
                    ...
                    className="text-[10px] font-black text-primary bg-primary/5 px-3 py-1.5 rounded-lg hover:bg-primary hover:text-white transition-all"
                  >
                    + 추가
                  </button>
                )}
              ...
              {showSubInput && (
                <div className="bg-white/50 rounded-2xl p-4 border border-primary/20 space-y-3 animate-fade-in shadow-inner">
                  <input
                    value={newSubContent}
                    ...
                    placeholder="URL 또는 메모 추가..."
                    className="w-full px-4 py-2 bg-white rounded-xl text-xs font-medium outline-none border border-outline-variant/30 focus:border-primary/30 transition-all font-inter"
                    autoFocus
                  />
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowSubInput(false)}
                      className="flex-1 py-2 bg-surface text-[10px] font-black uppercase text-on-surface-variant/40 rounded-lg hover:bg-surface-container transition-colors"
                    >
                      취소
                    </button>
                    <button
                      onClick={handleAddSubItem}
                      className="flex-[2] py-2 bg-primary text-white text-[10px] font-black uppercase rounded-lg shadow-lg shadow-primary/10 hover:bg-primary-600 transition-all"
                    >
                      저장
                    </button>
                  </div>
