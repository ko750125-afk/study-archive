        ...
        <div className="p-8 md:p-10">
          ...
          <div className="mb-10">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-[11px] font-black text-on-surface-variant/40 uppercase tracking-widest px-2">연관 리소스</h4>
              {!showSubInput && (
                <button
                  onClick={() => setShowSubInput(true)}
                  className="group flex items-center gap-2 px-5 py-2.5 bg-primary/10 text-primary rounded-xl text-xs font-black hover:bg-primary hover:text-white transition-all active:scale-95"
                >
            ...
            {showSubInput && (
              <div className="bg-surface-container-low/50 rounded-3xl p-6 mb-6 border-2 border-primary/20 space-y-4 animate-fade-in shadow-inner">
                <p className="text-[10px] font-black text-primary uppercase tracking-widest px-1">URL 또는 내용을 입력하세요</p>
                <div className="flex flex-col gap-3">
                  <input
                    value={newSubContent}
                    onChange={(e) => setNewSubContent(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && newSubContent.trim()) {
                        setSubItems([...subItems, { 
                          title: isUrl(newSubContent) ? "연관 링크" : "노트", 
                    ...
                    placeholder="https://example.com 또는 간단한 메모..."
                    className="w-full px-5 py-3 bg-white rounded-2xl text-sm font-medium outline-none border border-outline-variant/30 focus:border-primary/40 focus:ring-4 focus:ring-primary/5 transition-all"
                    autoFocus
                  />
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowSubInput(false)}
                      className="flex-1 px-4 py-2.5 bg-surface text-on-surface-variant/60 rounded-xl text-xs font-black uppercase hover:bg-surface-container transition-colors"
                    >
                      취소
                    </button>
                    <button
                      ...
                      className="flex-[2] px-4 py-2.5 bg-primary text-white rounded-xl text-xs font-black uppercase hover:bg-primary-600 shadow-lg shadow-primary/20 transition-all"
                    >
                      추가하기
                    </button>
                  </div>
